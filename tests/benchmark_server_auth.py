...
def run_server(processes=1):
  server = TotoServer(method_module='web_methods', port=9000, processes=processes, database='json', db_host='')
...
class TestWeb(unittest.TestCase):
  ...
  def test_method(self):
    ...
    headers = {'content-type': 'application/json', 'x-toto-session-id': session_id}
    req = urllib2.Request('http://127.0.0.1:9000/', json.dumps(req_data), headers)
    start = time()
