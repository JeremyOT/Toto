...
class MySQLdbSession():
  ...
  def save_state(self):
    if not self._verified:
      raise TotoError(ERROR_NOT_AUTHORIZED, "Not authorized")
...
class MySQLdbConnection():
  ...
  def create_account(self, user_id, password):
    if self.db.get("select account_id from account where user_id = %s", user_id):
      raise TotoError(ERROR_USER_ID_EXISTS, "User ID already in use.")
  ...
  def create_session(self, user_id, password, ttl=0):
    ...
    if not account:
      raise TotoError(ERROR_USER_NOT_FOUND, "Invalid user ID or password")
  ...
  def retrieve_session(self, session_id, hmac_data, data):
    ...
    if not session_data:
      raise TotoError(ERROR_INVALID_SESSION_ID, "Invalid session ID")
    ...
    if not hmac_data or hmac_data != base64.b64encode(hmac.new(str(session_data['user_id']), data, hashlib.sha1).digest()):
      raise TotoError(ERROR_INVALID_HMAC, "Invalid HMAC")
